...
function setupUserActions() {
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('delete-product-admin')) {
            deleteProduct(e.target.dataset.productId, e.target.closest('.admin-product-item'));
        }
    });
}
...
function setupAnimations() {
    ...
    document.querySelectorAll('.admin-product-item, .analytics-card').forEach(el => {
        observer.observe(el);
    });
}
...
function banUser(userId, userRow) {
    ...
    if (confirmation) {
        ...
        if (statusBadge) {
            ...
            statusBadge.style.background = 'linear-gradient(135deg, #dc3545, #c82333)';
        }
...
function deleteUser(userId, userRow) {
    const userName = userRow.querySelector('.user-name').textContent;
    const confirmation = confirm(`Are you sure you want to delete ${userName}? This action cannot be undone.`);
